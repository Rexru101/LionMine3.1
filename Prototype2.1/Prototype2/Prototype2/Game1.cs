...
//Cannot place mines in outer top left hex of the red zone on either side
...
>>>>>>> 75ee116e10d692e7886b7223d166f35605fdb0dc
        Player p1, p2;
        ...
        public void movePlayer(Player p, int row, int col)
        {
            ...
            for (int i = 1; i < p.getInventory().Length; i++)
            {
                justMovement = false;
                if (Player.Player1Turn)
                    calculateFlagDistance(p, new Point(row, col), p2.getInventory()[i]);
                else if (!Player.Player1Turn)
                    calculateFlagDistance(p, new Point(row, col), p1.getInventory()[i]);
        ...
        public bool bombsAreAdjacent(Player p)
        {
            for (int z = 1, plusZ = 1; z < p.getInventory().Length; z++)
            {
                while (z + plusZ < p.getInventory().Length)
                {
                    if (isMovementValid(p.getInventory()[z], p.getInventory()[z + plusZ].X, p.getInventory()[z + plusZ].Y))
                    {
                        return true;
                    }
                    plusZ++;
